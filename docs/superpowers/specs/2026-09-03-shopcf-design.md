...
variant_prices(
  ...
  amount_minor  INTEGER NOT NULL,          -- 最小货币单位整数
...
2. **多币种是独立定价而非汇率换算**。欧洲市场可定 €99 而非按汇率机械换算 $99。某币种价格缺失时在代码层回落到基准币种换算，数据结构不变。
...
- GBP 独立定价，不并入 EUR——英国脱欧后税制与定价习惯已与欧元区分离
- 语言与币种解耦：`en` 用户可手动切到 GBP 或 EUR，选择存 cookie
- 某 variant 缺少某币种价格时，回落到 USD 按配置汇率换算并向上取整到心理价位，回落逻辑在代码层，数据结构不变
...
| 模块 | 功能 |
|---|---|
| 商品管理 | 商品 CRUD、SKU/规格矩阵、多币种定价、库存调整（写流水）、上下架 |
...
1. **单元测试**：价格计算、多币种回落、库存扣减条件、订单状态机、hreflang / JSON-LD / sitemap 生成
